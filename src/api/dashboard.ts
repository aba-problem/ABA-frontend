...
/**
 * Rotates the credential for engines where the password is generated by an
 * external provider (currently: MongoDB only — the backend rejects this for
 * MySQL/SQLServer). Calls `POST /dashboard/bases/{id}/rotar-credencial`,
 * same strict rate limit as `getCredential` (5/hour per user).
 *
 * The provider regenerates BOTH username and password on reset (confirmed
 * against its real OpenAPI spec), not just the password — the response
 * includes both so callers can replace their local state fully.
 *
 * @param id - Database ID
 * @returns The new username and password (shown once), or error
 */
export async function rotateCredential(id: number) {
  return apiPost<{ usuario: string; password: string }>(`/dashboard/bases/${id}/rotar-credencial`)
}
